...
export function Dashboard() {
  ...
  const isReady = isMounted && status !== "loading";

  const shouldBlurBackground = !isReady || status === "unauthenticated";
  const shouldShowModal = shouldBlurBackground || Boolean(latestIssuedApiKey);
  ...
  useEffect(() => {
    ...
    const eventSource = new EventSource(
      `/api/stream/events?token=${encodeURIComponent(session.accessToken)}`,
    );
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-background">
      ...
      <div
        className={cn(
          "relative mx-auto flex min-h-screen w-full max-w-[1600px] flex-col px-4 py-4 transition duration-300 md:px-6",
          shouldBlurBackground && "pointer-events-none blur-xl",
        )}
      >
                        ...
                        <CardContent className="flex min-h-40 items-center justify-center p-8 text-center text-sm text-muted-foreground">
                          {!isReady
                            ...
                            : status === "authenticated" &&
                                resolvedConnectionStatus === "closed"
      ...
      {shouldShowModal ? <AuthModal /> : null}
    </div>
  );
}
